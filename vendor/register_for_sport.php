...
if ($event_date_row && strtotime($event_date_row['date']) < time()) {
    header('Location: ../sport_list.php?error=event_passed');
...
if ($places_data) {
    ...
    if ($max_participants !== NULL && $registered_count >= $max_participants) {
        header('Location: ../sport_list.php?error=no_places');
        exit(); // Stop further execution
    }
} else {
    // Handle case where sport is not found (optional, but good practice)
    header('Location: ../sport_list.php?error=sport_not_found');
    exit();
}
$places_stmt->close();

if ($row['COUNT(*)'] > 0) {
 header('Location: ../sport_list.php?error=already_registered');
    exit(); // Stop further execution
}
if (!empty($sport_id) && !empty($user_id)) {
    ...
    if ($stmt->execute()) {
 header('Location: ../sport_list.php?success=registered');
    } else {
